...
//		Boolean isValidRegistration = (Boolean) request.getAttribute("isValidRegistration");
		...
		if (errors.containsKey("NullStudentIdException")) {
			out.println("<input type='text' id='student_id' class='form-control border-danger' name='student_id' placeholder='학번 (숫자 10자)' value='" + registerStudentId + "' autofocus/>");
		...
		// Flush out all the Attributes that we received from this session
		session.invalidate();
	}
}