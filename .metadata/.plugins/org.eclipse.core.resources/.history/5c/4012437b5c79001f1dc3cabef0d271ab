...
public class StudentUpdateFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (!updateValidator.validate(updateParams)) {
			...
			showError(response, errors, stringId);
	...
	/**
	 * 사용자의 학생 정보 수정이 예외처리된 경우, 어떤 항목들이 어떤 규칙을 따라야 하는지 보여주는 Error 페이지로 이동합니다.
	 * 
	 * @param response 
	 * @param errors
	 * @throws IOException
	 */
	private void showError(ServletResponse response, Map<String, String> errors, String stringId) throws IOException {
	    ...
	    out.println("<div class='d-flex flex-column justify-content-center align-items-center h-50 w-50 bg-white rounded-3 p-5'>");
	    out.println("<h3 class='text-danger'>에러 메세지</h3>");
	    out.println("<p class='text-primary-emphasis'>학생 정보를 수정하는 도중 에러가 발생했습니다.</p>");
	    out.println("<div class='py-3 d-flex flex-column align-items-evenly justify-content-start'>");
	    
	    for (Map.Entry<String, String> entry : errors.entrySet()) {
	        out.println("<h2 class='fs-2'>" + entry.getValue() + "</h2>");
	    }
	    
	    out.println("</div>");
	    out.println("<a class='text-info fs-3' href='/readStudent.do?id=" + stringId + "'>다시 돌아가기</a>");
	    out.println("</div>");
	    out.println("</section>");
	    out.println("</main>");
	    out.println("</body>");
